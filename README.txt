This is a short project that uses Image Clasification inference to help make an every day interaction more efficient.The goal of training this model was to see the potential usefulness and accuracy of what hand controls could look like with a computer.My first thought (because I always listen to music while working) was to make controlling your music an easier task. All it takes is a camera. Using an enhanced version of my model, a user would simply have to hold out an open hand under a camera to play their music, close their hand into a fist to stop it, and make an L shape with their hand to skip to the next song etc. The possibilities of handcontrolling computer are endless, but I can imaging something like how Tony Stark interacts with Jarvis in Iron Man. 
...
2. change directory into python/project
3. run "imagenet.py --model=models/hands_final/resnet18.onnx --input_blob=input_0 --output_blob=output_0 --labels=data/hands_final/labels.txt /dev/inputs/js0"
